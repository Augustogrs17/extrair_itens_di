...
def criar_aba_resumo(wb_out, resumo):
    ...
    CARDS = [
        ("EFETIVADOS (TOTAL)", ef["total"], "0", "BDD7EE"),
        ("EFETIVADOS NO PRAZO", ef["pct_no_prazo"], "0.0%", "C6E0B4"),
        ("EFETIVADOS ATRASADOS", ef["pct_atrasados"], "0.0%", "F8CBAD"),
        ("ATRASO MÉDIO - EFETIVADOS (DIAS)", ef["atraso_medio_dias"], "0.0", "F8CBAD"),

        ("EM ABERTO (PRELIMINAR)", ab["total"], "0", "FFE699"),
        ("EM ABERTO NO PRAZO", ab["pct_no_prazo"], "0.0%", "C6E0B4"),
        ("EM ABERTO ATRASADOS", ab["pct_atrasados"], "0.0%", "F8CBAD"),
        ("ATRASO MÉDIO - EM ABERTO (DIAS)", ab["atraso_medio_dias"], "0.0", "F8CBAD"),
    ]
    ...
    for idx, (rotulo, valor, fmt, cor) in enumerate(CARDS):
        ...
        c_num.font = Font(bold=True, size=22, color="404040")
    ...
    data_cat = Reference(ws, min_col=2, min_row=linha_categoria_header, max_row=ultima_linha_categoria)
    labels_cat = Reference(ws, min_col=1, min_row=linha_categoria_header + 1, max_row=ultima_linha_categoria)
    bar.add_data(data_cat, titles_from_data=True)
    bar.set_categories(labels_cat)

    serie = bar.series[0]
    serie.graphicalProperties.solidFill = "F4B183"
    ...
    bar.height = 9
    bar.width = 13
    linha_grafico_categoria = linha_categoria_header
    ws.add_chart(bar, f"D{linha_grafico_categoria}")
