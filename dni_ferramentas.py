...
def calcular_resumo(ws, ultima_linha):
    """
    Calcula estatisticas de resumo (sem escrever na planilha):
    - contagens por status (coluna DEP) + EFETIVADOS + TOTAL
    - % efetivados no prazo, atraso medio (dias), atraso medio (%)

    'Efetivado' = valor da coluna DEP e' uma data.
    'No prazo'  = DIAS (recalculado) <= 0.

    Retorna dict com chaves: 'contagens' (lista de tuplas rotulo/valor)
    e 'kpis' (lista de tuplas rotulo/valor/formato).
    """
        ...
        if isinstance(dep_val, (datetime.date, datetime.datetime)):
            ...
            if isinstance(dias_val, (int, float)):
                if dias_val >= 0:
                    ...
                    atrasos_dias.append(-dias_val)

                estimativa = ws.cell(row=row, column=COL_ESTIMATIVA).value
                solicitacao = ws.cell(row=row, column=COL_SOLICITACAO).value
                if isinstance(estimativa, (datetime.date, datetime.datetime)) and \
                   isinstance(solicitacao, (datetime.date, datetime.datetime)):
                    est_d = estimativa.date() if isinstance(estimativa, datetime.datetime) else estimativa
                    sol_d = solicitacao.date() if isinstance(solicitacao, datetime.datetime) else solicitacao
                    prazo_planejado = (est_d - sol_d).days
                    if prazo_planejado > 0:
                        prazos_planejados.append(prazo_planejado)
        else:
    ...
    fora_prazo = efetivados - no_prazo
    pct_no_prazo = (no_prazo / efetivados) if efetivados > 0 else 0
    pct_fora_prazo = (fora_prazo / efetivados) if efetivados > 0 else 0

    atraso_medio_dias = sum(atrasos_dias) / len(atrasos_dias) if atrasos_dias else 0
    prazo_medio_planejado = (sum(prazos_planejados) / len(prazos_planejados)
                              if prazos_planejados else 0)
    atraso_medio_pct = (atraso_medio_dias / prazo_medio_planejado
                         if prazo_medio_planejado > 0 else 0)

    itens_contagens = [(s, contagens[s]) for s in STATUS_TEXTUAIS]
    itens_contagens.append(("EFETIVADOS", efetivados))
    itens_contagens.append(("TOTAL", total_itens))

    itens_kpis = [
        ("% EFETIVADOS NO PRAZO", pct_no_prazo, "0.00%"),
        ("EFETIVADOS FORA DO PRAZO (QTD)", fora_prazo, "0"),
        ("EFETIVADOS FORA DO PRAZO (%)", pct_fora_prazo, "0.00%"),
        ("ATRASO MÉDIO (DIAS)", round(atraso_medio_dias, 1), "0.0"),
        ("ATRASO MÉDIO (%)", atraso_medio_pct, "0.00%"),
    ]

    itens_prazo = [
        ("NO PRAZO", pct_no_prazo),
        ("FORA DO PRAZO", pct_fora_prazo),
    ]

    return {"contagens": itens_contagens, "kpis": itens_kpis, "prazo": itens_prazo}


def criar_aba_resumo(wb_out, resumo):
    """
    EN: Creates a "Resumo" sheet as a dashboard:
      - Top: 6 big KPI "cards" (merged cells, large font, colored fill)
          TOTAL, PRELIMINAR, EFETIVADOS, % NO PRAZO,
          EFETIVADOS FORA DO PRAZO (qtd), ATRASO MEDIO (dias)
      - Below: data tables (STATUS, PRAZO) feeding two charts placed
        side-by-side: pie (status distribution) and donut (no prazo x
        fora do prazo), both with right-side legend and % data labels.
    No bar chart (removed — mixed units were misleading).
    """
    ...
    # ================= EXTRAI VALORES PARA OS CARDS =================
    dict_contagens = dict(resumo["contagens"])
    dict_kpis = dict((r, v) for r, v, *_ in resumo["kpis"])
    dict_prazo = dict(resumo["prazo"])

    total = dict_contagens.get("TOTAL", 0)
    preliminar = dict_contagens.get("PRELIMINAR", 0)
    efetivados = dict_contagens.get("EFETIVADOS", 0)
    pct_no_prazo = dict_prazo.get("NO PRAZO", 0)
    fora_prazo_qtd = dict_kpis.get("EFETIVADOS FORA DO PRAZO (QTD)", 0)
    atraso_medio_dias = dict_kpis.get("ATRASO MÉDIO (DIAS)", 0)

    # ================= CARDS (DASHBOARD) =================
    # Grid 3 colunas x 2 linhas, cada card ocupa 2 colunas x 2 linhas
    # (1 linha para o numero grande, 1 linha para o rotulo)
    CARDS = [
        ("TOTAL DE PROJETOS", total, "0", "BDD7EE"),
        ("PRELIMINAR", preliminar, "0", "FFE699"),
        ("EFETIVADOS", efetivados, "0", "C6E0B4"),
        ("% EFETIVADOS NO PRAZO", pct_no_prazo, "0.0%", "C6E0B4"),
        ("FORA DO PRAZO (QTD)", fora_prazo_qtd, "0", "F8CBAD"),
        ("ATRASO MÉDIO (DIAS)", atraso_medio_dias, "0.0", "F8CBAD"),
    ]

    CARD_LARGURA_COLS = 3   # cada card ocupa 3 colunas
    CARD_ALTURA_NUM = 1     # linha do numero grande
    CARDS_POR_LINHA = 3
    ...
    for idx, (rotulo, valor, fmt, cor) in enumerate(CARDS):
        ...
        c_num.font = Font(bold=True, size=24, color="404040")
        c_num.alignment = Alignment(horizontal="center", vertical="center")
        c_num.fill = PatternFill(start_color=cor, end_color=cor, fill_type="solid")
        ...
        c_lbl.font = Font(bold=True, size=9, color="595959")
        c_lbl.alignment = Alignment(horizontal="center", vertical="center")
        c_lbl.fill = PatternFill(start_color=cor, end_color=cor, fill_type="solid")
    ...
    for k in range(n_linhas_cards):
        ws.row_dimensions[1 + k * 3].height = 36       # linha do numero
    ...
    def escrever_tabela(linha_inicio, titulo_col_a, titulo_col_b, itens, formatos=None):
        ws.cell(row=linha_inicio, column=1, value=titulo_col_a)
        ws.cell(row=linha_inicio, column=2, value=titulo_col_b)
        for col in (1, 2):
        ...
        for i, item in enumerate(itens):
            ...
            c_rot = ws.cell(row=row, column=1, value=rotulo)
    ...
    linha_status_header = linha_apos_cards + 1
    ultima_linha_status = escrever_tabela(linha_status_header, "STATUS", "QUANTIDADE", resumo["contagens"])
    ...
    # Paleta suave (elegante) para os graficos
    PALETA = ["8FAADC", "F4B183", "A9D18E", "C9C9C9", "FFD966", "B4A7D6", "9DC3E6"]
    ...
    data = Reference(ws, min_col=2, min_row=linha_status_header, max_row=ultima_linha_pizza)
    labels = Reference(ws, min_col=1, min_row=linha_status_header + 1, max_row=ultima_linha_pizza)
    ...
    pie.height = 9
    pie.width = 13
    linha_graficos = linha_apos_cards
    ws.add_chart(pie, f"D{linha_graficos}")
    ...
    data_prazo = Reference(ws, min_col=2, min_row=linha_prazo_header, max_row=ultima_linha_prazo)
    labels_prazo = Reference(ws, min_col=1, min_row=linha_prazo_header + 1, max_row=ultima_linha_prazo)
    donut.add_data(data_prazo, titles_from_data=True)
    donut.set_categories(labels_prazo)

    cores_prazo = ["A9D18E", "F4B183"]  # verde (no prazo) / laranja (fora do prazo)
    ...
    donut.height = 9
    donut.width = 13
    # Lado a lado com a pizza (~13 colunas de largura cada => +9 colunas)
    ws.add_chart(donut, f"M{linha_graficos}")
...
def criar_aba_efetivados(wb_out, ws_origem, ultima_linha):
    """
    EN: Creates a second sheet "Projetos Efetivados" with only the rows
    where DEP (col C) is a date (i.e. the project has been delivered),
    keeping the most relevant columns for analysis.
    """
    # Colunas relevantes (1-based, na planilha origem) e seus titulos
    colunas_relevantes = [
        (1, "PROJETO"),
        (3, "DATA EFETIVAÇÃO"),
        (15, "SOLICITAÇÃO"),
    ...
    for j, (_, titulo) in enumerate(colunas_relevantes, start=1):
        cell = ws_ef.cell(row=1, column=j, value=titulo)
    ...
    for row in range(LINHA_PRIMEIRA_DADO, ultima_linha + 1):
        ...
        for j, (col_origem, _) in enumerate(colunas_relevantes, start=1):
            valor = ws_origem.cell(row=row, column=col_origem).value
            cell = ws_ef.cell(row=linha_ef, column=j, value=valor)
            cell.alignment = Alignment(horizontal="center", vertical="center")
            if col_origem in (3, 15, 16):  # datas
                cell.number_format = "dd/mm/yy"
        linha_ef += 1

    ultima_linha_ef = linha_ef - 1

    if ultima_linha_ef >= 1:
        fina = Side(style="thin", color="BFBFBF")
        borda = Border(left=fina, right=fina, top=fina, bottom=fina)
        for row in range(1, max(ultima_linha_ef, 1) + 1):
            for col in range(1, len(colunas_relevantes) + 1):
                ws_ef.cell(row=row, column=col).border = borda

        if ultima_linha_ef >= 2:
            ws_ef.auto_filter.ref = (
                f"A1:{get_column_letter(len(colunas_relevantes))}{ultima_linha_ef}"
            )
            ws_ef.freeze_panes = "A2"

            # --- Formatacao condicional: atraso (DIAS) ---
            # DIAS < 0 -> atrasado (vermelho), >= 0 -> no prazo/adiantado (verde)
            from openpyxl.formatting.rule import CellIsRule
            col_dias_ef = 5  # "DIAS (ATRASO)" e' a 5a coluna em colunas_relevantes
            rng = f"{get_column_letter(col_dias_ef)}2:{get_column_letter(col_dias_ef)}{ultima_linha_ef}"
            ws_ef.conditional_formatting.add(
                rng,
    ...
    for col, (_, titulo) in enumerate(colunas_relevantes, start=1):
        ws_ef.column_dimensions[get_column_letter(col)].width = max(12, len(titulo) + 2)

    print(f"Aba 'Projetos Efetivados': {ultima_linha_ef} projeto(s)")
...
def extrair_itens_di(ws):
    ...
    diag = "ok" if itens else f"tabela encontrada (header linha {linha_header}) mas 0 itens com SIT=DI e TIPO=COMP"
    return itens, diag
