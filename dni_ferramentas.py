...
def calcular_resumo(ws, ultima_linha):
    ...
    for row in range(LINHA_PRIMEIRA_DADO, ultima_linha + 1):
        ...
        # Conta somente projetos ativos (exclui CANCELADO e EFETIVADOS)
        dep_str = str(dep_val).strip().upper() if dep_val else ""
        _ativo = not isinstance(dep_val, (datetime.date, datetime.datetime)) \
                 and dep_str != "CANCELADO"
        if _ativo:
...
def criar_aba_resumo(wb_out, resumo):
    """
    EN: Light-mode professional dashboard.
    Layout (fixed grid, no overlaps):
      Cols A-E  : 8 KPI cards stacked vertically (rows 2-19)
      Cols F-M  : Chart 1 (pizza) top-left    anchor F1
      Cols N-U  : Chart 2 (donut) top-right   anchor N1
      Cols F-U  : Chart 3 (barras) bottom     anchor F21
      Cols V-W  : Auxiliary tables (off main view, feed charts)
    Fonts: labels=12, values=16, highlight values (total)=18
    """
    ...
    fill_sheet = PatternFill(start_color=BG_SHEET, end_color=BG_SHEET, fill_type="solid")
    fill_card  = PatternFill(start_color=BG_CARD,  end_color=BG_CARD,  fill_type="solid")

    for r in range(1, 55):
        for c in range(1, 40):
            ws.cell(row=r, column=c).fill = fill_sheet

    # ── Column widths ──────────────────────────────────────────────────
    ws.column_dimensions["A"].width = 1.5    # left margin
    ws.column_dimensions["B"].width = 2.5    # accent strip
    ws.column_dimensions["C"].width = 28     # label (font 12 needs more space)
    ws.column_dimensions["D"].width = 12     # value
    ws.column_dimensions["E"].width = 2      # gap before charts
    for l in ["F","G","H","I","J","K","L","M"]:  # chart zone 1 (~8 cols = ~56pt wide)
        ws.column_dimensions[l].width = 8
    ws.column_dimensions["N"].width = 1.5    # gap between charts
    for l in ["O","P","Q","R","S","T","U","V"]:  # chart zone 2
        ws.column_dimensions[l].width = 8
    ws.column_dimensions["W"].width = 26     # aux table col A
    ws.column_dimensions["X"].width = 16     # aux table col B

    # ── Row heights ────────────────────────────────────────────────────
    ws.row_dimensions[1].height  = 8      # top padding
    ws.row_dimensions[2].height  = 16     # group header EFETIVADOS
    for r in range(3, 7):
        ws.row_dimensions[r].height = 30  # KPI rows 1-4 (efetivados)
    ws.row_dimensions[7].height  = 12    # gap
    ws.row_dimensions[8].height  = 16    # group header EM ABERTO
    for r in range(9, 13):
        ws.row_dimensions[r].height = 30  # KPI rows 5-8 (em aberto)
    ws.row_dimensions[13].height = 8     # bottom padding

    ef = resumo["efetivados"]
    ab = resumo["em_aberto"]

    def _borda(cor=BORDER):
        s = Side(style="thin", color=cor)
        return Border(left=s, right=s, top=s, bottom=s)

    # ── Group header ───────────────────────────────────────────────────
    def _group_hdr(row, texto, cor):
        ws.merge_cells(f"B{row}:D{row}")
        for col in (2, 3, 4):
            ws.cell(row=row, column=col).fill = \
                PatternFill(start_color=cor, end_color=cor, fill_type="solid")
        c = ws.cell(row=row, column=2, value=f"  {texto}")
        c.font      = Font(bold=True, size=10, color="FFFFFF")
        c.alignment = Alignment(horizontal="left", vertical="center")

    # ── KPI card ───────────────────────────────────────────────────────
    def _kpi(row, label, valor, fmt, cor, font_size=16):
        acc = ws.cell(row=row, column=2)
        acc.fill   = PatternFill(start_color=cor, end_color=cor, fill_type="solid")
        acc.border = _borda(cor)

        lbl = ws.cell(row=row, column=3, value=label)
        lbl.font      = Font(size=12, color=CINZA_T)
        lbl.alignment = Alignment(horizontal="left", vertical="center")
        lbl.fill      = fill_card
        lbl.border    = _borda()

        val = ws.cell(row=row, column=4, value=valor)
        val.number_format = fmt
        val.font      = Font(bold=True, size=font_size, color=cor)
        val.alignment = Alignment(horizontal="right", vertical="center")
        val.fill      = fill_card
        val.border    = _borda()

    # Group 1: EFETIVADOS
    _group_hdr(2, "EFETIVADOS", AZUL_ESC)
    _kpi(3,  "Total Efetivados",            ef["total"],             "0",    TEAL,    font_size=18)
    _kpi(4,  "No Prazo",                    ef["pct_no_prazo"],      "0.0%", VERDE,   font_size=16)
    _kpi(5,  "Atrasados",                   ef["pct_atrasados"],     "0.0%", LARANJA, font_size=16)
    _kpi(6,  "Atraso Médio (dias)",         ef["atraso_medio_dias"], "0.0",  LARANJA, font_size=16)

    # Group 2: EM ABERTO
    _group_hdr(8, "EM ABERTO (PRELIMINAR)", AZUL_ESC)
    _kpi(9,  "Total em Aberto",             ab["total"],             "0",    TEAL,    font_size=18)
    _kpi(10, "No Prazo",                    ab["pct_no_prazo"],      "0.0%", VERDE,   font_size=16)
    _kpi(11, "Atrasados",                   ab["pct_atrasados"],     "0.0%", LARANJA, font_size=16)
    _kpi(12, "Atraso Médio (dias)",         ab["atraso_medio_dias"], "0.0",  LARANJA, font_size=16)

    # ── Auxiliary tables (cols W-X) ────────────────────────────────────
    COL_T = 23  # W

    def _tabela_aux(linha, h_a, h_b, itens):
        bd = _borda()
        for j, val in enumerate([h_a, h_b]):
            c = ws.cell(row=linha, column=COL_T + j, value=val)
            c.font  = Font(bold=True, color="FFFFFF", size=9)
            c.fill  = PatternFill(start_color=AZUL_ESC, end_color=AZUL_ESC, fill_type="solid")
            c.alignment = Alignment(horizontal="center", vertical="center")
            c.border = bd
        r = linha + 1
        for item in itens:
            rot, valor = item[0], item[1]
            fmt = item[2] if len(item) > 2 else None
            cr = ws.cell(row=r, column=COL_T,   value=rot)
            cv = ws.cell(row=r, column=COL_T+1, value=valor)
            if fmt: cv.number_format = fmt
            for cell in (cr, cv):
                cell.font      = Font(size=8, color=AZUL_ESC)
                cell.fill      = fill_card
                cell.border    = bd
    ...
    l_st = 1
    u_st = _tabela_aux(l_st, "STATUS", "QTD", resumo["contagens"])
    l_pr = u_st + 2
    u_pr = _tabela_aux(l_pr, "PRAZO", "%", [(r, v, "0.00%") for r, v in resumo["prazo"]])
    l_ca = u_pr + 2
    u_ca = _tabela_aux(l_ca, "CATEGORIA", "ATRASADOS",
                        resumo["atrasados_categoria"] or [("(nenhum)", 0)])
    l_li = u_ca + 2
    _tabela_aux(l_li, "LÍDER TÉCNICO", "PROJ. ATIVOS",
                resumo["projetos_por_lider"] or [("(sem líder)", 0)])

    # ── Chart helpers ──────────────────────────────────────────────────
    PALETA = [TEAL, "5B9BD5", LARANJA, AMARELO, "9B8CF2", "6B9080", VERDE]

    def _dl_pct():
        dl = DataLabelList()
        dl.showPercent = True; dl.numFmt = "0.0%"
        dl.showVal = False; dl.showCatName = False
        dl.showSerName = False; dl.showLegendKey = False
        return dl

    # ── Chart 1: Pizza — Status (top-left, anchor F1) ─────────────────
    pie = PieChart()
    pie.title = "Distribuição de Status dos Projetos"
    u_pizza = u_st - 1  # exclude TOTAL
    pie.add_data(Reference(ws, min_col=COL_T+1, min_row=l_st,   max_row=u_pizza),
                 titles_from_data=True)
    pie.set_categories(Reference(ws, min_col=COL_T, min_row=l_st+1, max_row=u_pizza))
    for i in range(u_pizza - l_st):
        pt = DataPoint(idx=i)
        pt.graphicalProperties.solidFill = PALETA[i % len(PALETA)]
        pie.series[0].data_points.append(pt)
    dl_pie = _dl_pct()
    dl_pie.showCatName = True
    pie.dataLabels = dl_pie
    pie.legend = None
    pie.height = 10.5  # ~20 rows of 30pt
    pie.width  = 15    # ~8 cols of 8pt
    ws.add_chart(pie, "F1")

    # ── Chart 2: Donut — Prazo (top-right, anchor O1) ─────────────────
    donut = PieChart()
    donut.title = "Efetivados: No Prazo × Atrasados"
    try: donut.type = "doughnut"
    except: pass
    donut.add_data(Reference(ws, min_col=COL_T+1, min_row=l_pr,   max_row=u_pr),
                   titles_from_data=True)
    donut.set_categories(Reference(ws, min_col=COL_T, min_row=l_pr+1, max_row=u_pr))
    for i, cor in enumerate([VERDE, LARANJA]):
        pt = DataPoint(idx=i)
        pt.graphicalProperties.solidFill = cor
        donut.series[0].data_points.append(pt)
    donut.dataLabels = _dl_pct()
    donut.legend.position = "b"
    donut.legend.overlay  = False
    donut.height = 10.5
    donut.width  = 15
    ws.add_chart(donut, "O1")

    # ── Chart 3: Barras horizontais — Categoria (bottom, full-width, anchor F21) ──
    bar = BarChart()
    bar.type  = "bar"
    bar.title = "Projetos Atrasados por Categoria (Efetivados + Em Aberto)"
    bar.style = 10
    bar.add_data(Reference(ws, min_col=COL_T+1, min_row=l_ca,   max_row=u_ca),
                 titles_from_data=True)
    bar.set_categories(Reference(ws, min_col=COL_T, min_row=l_ca+1, max_row=u_ca))
    bar.series[0].graphicalProperties.solidFill = TEAL
    dl_bar = DataLabelList()
    dl_bar.showVal = True
    bar.dataLabels = dl_bar
    bar.legend = None
    bar.height = 10
    bar.width  = 30   # spans both chart columns (F to V)
    ws.add_chart(bar, "F21")
...
def criar_aba_preliminares(wb_out, ws_origem, ultima_linha):
    """EN: Creates "Projetos Preliminares" sheet — only rows where DEP == 'PRELIMINAR'.
    DEP column formatted as data abreviada (dd/mm/yy) for when it becomes a date."""
    colunas = [
        (1,  "PROJETO"),
        (3,  "DEP"),
        (2,  "DIAS"),
    ...
    for row in range(LINHA_PRIMEIRA_DADO, ultima_linha + 1):
        dep_val = ws_origem.cell(row=row, column=COL_DEP).value
        # Filtro: apenas DEP == "PRELIMINAR" (texto, case-insensitive)
        if not (isinstance(dep_val, str) and dep_val.strip().upper() == "PRELIMINAR"):
            continue
        linhas.append([ws_origem.cell(row=row, column=col).value for col, _ in colunas])
    _formatar_aba_projetos(ws_pre, colunas, linhas,
                            col_assunto_local=6, col_dias_local=3,
                            colunas_data_local={2, 4, 5})  # DEP(2)+ESTIMATIVA(4)+SOLICITAÇÃO(5)
