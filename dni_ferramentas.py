...
def criar_aba_resumo(wb_out, resumo):
    """
    EN: Professional light-mode dashboard.
    Layout: cols A-D = 8 KPI cards vertical; cols F-N = 3 charts stacked;
            cols O-P = auxiliary data tables (off main view, feed charts).
    Palette: white cards, Metalfrio navy/teal accents, high-contrast text.
    """
    ...
    c = ws.cell(row=2, column=2, value="DASHBOARD DNI  |  PROJETOS")
    c.font      = Font(bold=True, size=14, color=AZUL_ESC)
    ...
    def _kpi(row, label, valor, fmt, cor):
        # accent strip
        ...
        lbl.font      = Font(size=9, color=CINZA_T)
        ...
        val.font      = Font(bold=True, size=13, color=cor)
        val.alignment = Alignment(horizontal="right", vertical="center")
        val.fill      = fill_card
        val.border    = _borda()

    _group_hdr(4,  "EFETIVADOS",           AZUL_ESC)
    _kpi(5,  "Total Efetivados",           ef["total"],             "0",    TEAL)
    _kpi(6,  "No Prazo",                   ef["pct_no_prazo"],      "0.0%", VERDE)
    _kpi(7,  "Atrasados",                  ef["pct_atrasados"],     "0.0%", LARANJA)
    _kpi(8,  "Atraso Médio (dias)",        ef["atraso_medio_dias"], "0.0",  LARANJA)

    _group_hdr(10, "EM ABERTO (PRELIMINAR)", AZUL_ESC)
    _kpi(11, "Total em Aberto",            ab["total"],             "0",    TEAL)
    _kpi(12, "No Prazo",                   ab["pct_no_prazo"],      "0.0%", VERDE)
    _kpi(13, "Atrasados",                  ab["pct_atrasados"],     "0.0%", LARANJA)
    _kpi(14, "Atraso Médio (dias)",        ab["atraso_medio_dias"], "0.0",  LARANJA)

    # Auxiliary tables (cols O-P, off main view)
    COL_T = 15

    def _tabela_aux(linha, h_a, h_b, itens):
        fh = Font(bold=True, color="FFFFFF", size=8)
        fill_h = PatternFill(start_color=AZUL_ESC, end_color=AZUL_ESC, fill_type="solid")
        bd = _borda()
        for j, val in enumerate([h_a, h_b]):
            c = ws.cell(row=linha, column=COL_T + j, value=val)
            c.font = fh; c.fill = fill_h
    ...
    u_pizza = u_st - 1
    pie.add_data(Reference(ws, min_col=COL_T+1, min_row=l_st,   max_row=u_pizza), titles_from_data=True)
    ...
    dl.showVal = False; dl.showCatName = True
    ...
    donut.add_data(Reference(ws, min_col=COL_T+1, min_row=l_pr,   max_row=u_pr), titles_from_data=True)
    ...
    dl2 = DataLabelList()
    dl2.showPercent = True; dl2.numFmt = "0.0%"
    dl2.showVal = False; dl2.showCatName = False
    dl2.showSerName = False; dl2.showLegendKey = False
    donut.dataLabels = dl2
    donut.legend.position = "r"; donut.legend.overlay = False
    donut.height = 10; donut.width = 16
    ...
    bar.style = 10
    bar.add_data(Reference(ws, min_col=COL_T+1, min_row=l_ca,   max_row=u_ca), titles_from_data=True)
