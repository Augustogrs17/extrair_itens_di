...
# RGB(11, 29, 47)  -> #0B1D2F  azul marinho logo (print 2)
# RGB(28, 186, 190) -> #1CBABE teal/verde logo    (print 1)
AZUL_METALFRIO  = "0B1D2F"
VERDE_METALFRIO = "1CBABE"
COL_SOLICITACAO = 15
COL_ESTIMATIVA = 16
...
def criar_aba_resumo(wb_out, resumo):
    """
    EN: Premium dark-mode dashboard "Resumo" sheet.

    Layout:
      - Column A-C  : 8 KPI cards stacked vertically (dark bg, teal numbers)
      - Column D-M  : 3 charts stacked vertically (status pie, prazo donut,
                      categoria bar chart)
      - Column N-O  : auxiliary data tables (off-screen, feed the charts)

    Color palette (Metalfrio identity):
      BG_DARK   = #0B1D2F  (deep navy — from logo)
      TEAL      = #1CBABE  (Metalfrio teal — from logo)
      BG_CARD   = #111F2E  (slightly lighter navy for cards)
      BG_SEP    = #0D2236  (separator between KPI groups)
      BRANCO    = #F0F4F8  (primary text)
      CINZA     = #8BA0B4  (secondary labels)
      LARANJA   = #E8805A  (alert — atrasados)
      VERDE     = #3DD68C  (ok — no prazo)
      AMARELO   = #F4C430  (warning — em aberto)
    """
    ...
    ws = wb_out.create_sheet("Resumo", 0)
    ...
    # ── Fill the entire visible area with BG_DARK ──────────────────────
    ...
    fill_sep  = PatternFill(start_color=BG_SEP,  end_color=BG_SEP,  fill_type="solid")
    ...
    # ── Column widths ──────────────────────────────────────────────────
    # A: label col, B: spacer, C: value col — each KPI card spans A:C
    ws.column_dimensions["A"].width = 2    # left margin
    ws.column_dimensions["B"].width = 22   # label
    ws.column_dimensions["C"].width = 12   # value
    ws.column_dimensions["D"].width = 2    # divider
    for col_letter in ["E","F","G","H","I","J","K","L","M"]:
        ws.column_dimensions[col_letter].width = 7
    ws.column_dimensions["N"].width = 24   # aux tables (off-screen)
    ws.column_dimensions["O"].width = 14

    # ── Row heights ────────────────────────────────────────────────────
    ws.row_dimensions[1].height  = 6    # top padding
    # KPI rows: each card = 3 rows (top-pad + value+label merged + bottom-pad)
    # 8 cards + 1 separator = 8*3 + 3 = 27 data rows, starting at row 2
    for r in range(2, 50):
    ...
    # (label, value, number_format, value_color)
    CARDS = [
        # ── EFETIVADOS ──
        ("EFETIVADOS (TOTAL)",             ef["total"],             "0",    TEAL),
        ("EFETIVADOS NO PRAZO",            ef["pct_no_prazo"],      "0.0%", VERDE),
        ...
        ("EM ABERTO NO PRAZO",             ab["pct_no_prazo"],      "0.0%", VERDE),
        ("EM ABERTO ATRASADOS",            ab["pct_atrasados"],     "0.0%", LARANJA),
        ("ATRASO MÉDIO EM ABERTO (DIAS)",  ab["atraso_medio_dias"], "0.0",  LARANJA),
    ]

    fina = Side(style="thin", color="1A3550")
    borda_card = Border(left=fina, right=fina, top=fina, bottom=fina)

    def _escrever_kpi(row_inicio, label, valor, fmt, cor_valor, fill):
        """Writes a single KPI card spanning cols B-C, rows row_inicio+0/+1."""
        # Value cell (col C, large font)
        c_val = ws.cell(row=row_inicio, column=3, value=valor)
        c_val.number_format = fmt
        c_val.font = Font(bold=True, size=16, color=cor_valor)
        c_val.alignment = Alignment(horizontal="right", vertical="center")
        c_val.fill = fill
        c_val.border = borda_card

        # Label cell (col B)
        c_lbl = ws.cell(row=row_inicio, column=2, value=label)
        c_lbl.font = Font(size=9, color=CINZA)
        c_lbl.alignment = Alignment(horizontal="left", vertical="center")
        c_lbl.fill = fill
        c_lbl.border = borda_card

        # Left accent cell (col A) — teal strip
        c_acc = ws.cell(row=row_inicio, column=1)
        c_acc.fill = PatternFill(start_color=cor_valor, end_color=cor_valor, fill_type="solid")

    # Write title header
    ws.row_dimensions[2].height = 22
    ws.merge_cells("B2:C2")
    c_title = ws.cell(row=2, column=2, value="DASHBOARD DNI — PROJETOS")
    c_title.font = Font(bold=True, size=13, color=TEAL)
    c_title.alignment = Alignment(horizontal="left", vertical="center")
    c_title.fill = fill_dark

    # Group header: EFETIVADOS
    ws.row_dimensions[3].height = 14
    ws.merge_cells("A3:C3")
    c_grp1 = ws.cell(row=3, column=1, value="  EFETIVADOS")
    c_grp1.font = Font(bold=True, size=8, color=BRANCO)
    c_grp1.alignment = Alignment(horizontal="left", vertical="center")
    c_grp1.fill = fill_sep

    # KPI rows start at row 4
    LINHA_KPI_START = 4
    for idx, (label, valor, fmt, cor) in enumerate(CARDS):
        row = LINHA_KPI_START + idx

        # Separator between groups (after card 4)
        if idx == 4:
            row += 1  # push em-aberto group down one row for visual gap
            ws.row_dimensions[row - 1].height = 8
            ws.merge_cells(f"A{row-1}:C{row-1}")
            c_sep = ws.cell(row=row - 1, column=1, value="  EM ABERTO (PRELIMINAR)")
            c_sep.font = Font(bold=True, size=8, color=BRANCO)
            c_sep.alignment = Alignment(horizontal="left", vertical="center")
            c_sep.fill = fill_sep

        fill = fill_card
        ws.row_dimensions[row].height = 22
        _escrever_kpi(row, label, valor, fmt, cor, fill)

    # ── Auxiliary data tables (cols N-O, off main view) ────────────────
    COL_T = 14  # N

    def _tabela_aux(linha, header_a, header_b, itens):
        fonte_h = Font(bold=True, color=BRANCO, size=8)
        fill_h  = PatternFill(start_color="1A3550", end_color="1A3550", fill_type="solid")
        borda_t = Border(left=fina, right=fina, top=fina, bottom=fina)
        for col, val in [(COL_T, header_a), (COL_T+1, header_b)]:
            c = ws.cell(row=linha, column=col, value=val)
            c.font = fonte_h; c.fill = fill_h
            c.alignment = Alignment(horizontal="center", vertical="center")
            c.border = borda_t
        r = linha + 1
        for rotulo, valor, *rest in itens:
            fmt = rest[0] if rest else None
            cr = ws.cell(row=r, column=COL_T, value=rotulo)
            cv = ws.cell(row=r, column=COL_T+1, value=valor)
            if fmt: cv.number_format = fmt
            for cell in (cr, cv):
                cell.font = Font(color=BRANCO, size=8)
                cell.fill = fill_card
                cell.border = borda_t
    ...
    linha_status = 1
    ultima_status = _tabela_aux(linha_status, "STATUS", "QTD", resumo["contagens"])

    linha_prazo = ultima_status + 2
    itens_prazo = [(r, v, "0.00%") for r, v in resumo["prazo"]]
    ultima_prazo = _tabela_aux(linha_prazo, "PRAZO", "%", itens_prazo)

    linha_cat = ultima_prazo + 2
    itens_cat = resumo["atrasados_categoria"] or [("(nenhum)", 0)]
    ultima_cat = _tabela_aux(linha_cat, "CATEGORIA", "ATRASADOS", itens_cat)

    linha_lider = ultima_cat + 2
    itens_lider = resumo["projetos_por_lider"] or [("(sem líder)", 0)]
    _tabela_aux(linha_lider, "LÍDER TÉCNICO", "PROJETOS", itens_lider)
        ...
        if chart.legend:
            chart.legend.textProperties = RichText(
                p=[Paragraph(pPr=ParagraphProperties(
                    defRPr=CharacterProperties(solidFill=CINZA)))]
            )
        for ax in ("x_axis", "y_axis"):
            a = getattr(chart, ax, None)
            if a:
                a.txPr = RichText(p=[Paragraph(pPr=ParagraphProperties(
                    defRPr=CharacterProperties(solidFill=CINZA)))])

    def _label_white(dl):
        dl.txPr = RichText(p=[Paragraph(pPr=ParagraphProperties(
            defRPr=CharacterProperties(solidFill=BRANCO, b=True)))])

    # Paleta de fatias para o gráfico de pizza (teal-based)
    PALETA_PIE = [TEAL, AZUL, LARANJA, AMARELO, "9B8CF2", "6B7280", VERDE]

    # ── Chart 1: Pizza — status ────────────────────────────────────────
    pie = PieChart()
    pie.title = "Status dos Projetos"
    ultima_pizza = ultima_status - 1  # exclude TOTAL row
    data_p = Reference(ws, min_col=COL_T+1, min_row=linha_status, max_row=ultima_pizza)
    lbl_p  = Reference(ws, min_col=COL_T,   min_row=linha_status+1, max_row=ultima_pizza)
    pie.add_data(data_p, titles_from_data=True)
    pie.set_categories(lbl_p)
    for i in range(ultima_pizza - linha_status):
        pt = DataPoint(idx=i)
        pt.graphicalProperties.solidFill = PALETA_PIE[i % len(PALETA_PIE)]
    ...
    # ── Chart 2: Donut — prazo efetivados ─────────────────────────────
    donut = PieChart()
    donut.title = "Efetivados: No Prazo x Atrasados"
    try: donut.type = "doughnut"
    except: pass
    data_d = Reference(ws, min_col=COL_T+1, min_row=linha_prazo, max_row=ultima_prazo)
    lbl_d  = Reference(ws, min_col=COL_T,   min_row=linha_prazo+1, max_row=ultima_prazo)
    donut.add_data(data_d, titles_from_data=True)
    donut.set_categories(lbl_d)
    for i, cor in enumerate([VERDE, LARANJA]):
        pt = DataPoint(idx=i)
    ...
    _label_white(donut.dataLabels)
    donut.legend.position = "r"
    donut.legend.overlay = False
    _dark_chart(donut)
    donut.height = 9
    donut.width  = 16
    ws.add_chart(donut, "E14")

    # ── Chart 3: Barras horizontais — atrasados por categoria ──────────
    bar = BarChart()
    bar.type  = "bar"
    bar.title = "Atrasados por Categoria"
    bar.style = 10
    data_b = Reference(ws, min_col=COL_T+1, min_row=linha_cat, max_row=ultima_cat)
    lbl_b  = Reference(ws, min_col=COL_T,   min_row=linha_cat+1, max_row=ultima_cat)
    bar.add_data(data_b, titles_from_data=True)
    bar.set_categories(lbl_b)
    bar.series[0].graphicalProperties.solidFill = TEAL
    bar.dataLabels = DataLabelList()
    bar.dataLabels.showVal = True
    _label_white(bar.dataLabels)
    bar.legend = None
    _dark_chart(bar)
    bar.height = 9
    bar.width  = 16
